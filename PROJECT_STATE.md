...
> **🎯 Now (08/07/2026): Map redesign is DONE + validated; the auth bug is fixed for good.** The whole map was rebuilt on **Mapbox** ([design](docs/design/map_engine_redesign.md) + [map_emoji_pins.md](docs/design/map_emoji_pins.md)): plot **places** (not one pin per event — `src/lib/mapPlaces.ts`), render with a native symbol layer, **emoji pins as bundled Twemoji-on-disc images** (`assets/mappins/` + `src/utils/mapPinImages.ts`), **native collision de-clutter** (`iconAllowOverlap:false` + `symbolSortKey` = notability tiers), snappy select, and selected-place persistence on zoom-out. Kevin validated it on **staging build #26** (delivered via OTA). Also fixed en route: the **geocode regression** (Places API New) and the events-empty issue.
>
...
>
> **⚠️ IMPORTANT state gap — the emoji map is NOT on the `staging` branch yet.** It lives on branch **`feat/map-emoji-icons`** (4 commits) + was delivered to build #26 only as **OTAs**. `origin/staging`'s embedded `MapboxPlacesMap` is still the older text-emoji version (renders blank on device). **So a native build or prod promotion from `staging` today would ship the blank-emoji map.**
> - **Next step (Kevin's go):** merge `feat/map-emoji-icons` → `staging` → deploy-staging builds a fresh native staging app that *embeds* the emoji map (build #27). Confirm on-device, then promote **staging → main** (gated).
> - **Prod promotion carries:** region model (migr 150) + recurrence collapse (migr 152) + Mapbox emoji map + login crash fix + geocode fix + env-isolation + fingerprint policy. Prod is behind by all of these. **Re-run the coord/geocode backfills on prod after promotion** (prod events are at ~0% coords — Tier 3, needs approval).
>
...
### In flight
- **`feat/map-emoji-icons` (4 commits) — emoji map pins + de-clutter + selection fix.** Delivered to **staging build #26 as OTAs** (rtv `7bef7ff`), validated on-device by Kevin. **NOT yet merged to the `staging` branch**, so the branch's embedded map is stale (text-emoji → blank on device). Next: merge → deploy-staging builds native #27 that embeds it → then gated prod promotion. Prod-promotion follow-up: re-run coord/geocode backfills on prod (Tier 3).
- Zombie **Deploy #61** stuck `queued` (GitHub-side, uncancellable via API) — superseded by the next staging deploy.
