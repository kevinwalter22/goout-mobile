...
**Last updated:** 08/16/2026 (Ingestion restored, curated, and self-monitoring. The ~2-month API blackout root-caused & fixed to prod [`next_fetch_partition` overflow + coordinator auth 401, #123]; notability-gated **Ticketmaster** live [first Portland crawl 108→106, 2 tributes dropped, #125]; rows-based **source-liveness monitor** watching all enabled sources [#126]; **`complete_normalization_job`** enum-cast regression fixed [jobs were finalizing ~29 min late via a sweep — now instant, #129/migration 157]. All shipped through the gated pipeline with Kevin's prod approvals.)
**Owner:** Kevin Walter
...
|------|----------|-----------|------------|
| [09/02/2026] | **PROD PROMOTION — `v1.0.0-prod.48`: Wave 1 device fixes + region stack + Wave 2 curation calibration** (migs 174–183, one gated staging→main bundle). Wave 2 = **model-town fix** + **de-biased region-relative eligibility** (mig 180, local-love term surfaces gems like Bellvale) + **notable-first order** (mig 181) + **distance-aware local-first selection** (mig 182, per-intent proximity decay: food/drink TIGHT 6mi, destinations WIDE 30mi) + **local civic/rec floor + seasonal carousel hide** (mig 183). Deploy runs a one-time corrected re-score (1,570 activities) + all-region blend, guarded by an `app_config` marker. Verified on prod. | Carousels read like a local's list; `blended_notability` kept as the sprawl/scorecard threshold, proximity + notable-first in SEPARATE columns. | Kevin (calibration + prod gate), Claude |
| [09/02/2026] | **Plan B — user-event photo-bubble pins (V1), proven.** User event shows as its photo in a fixed-size circular pin (mig 184 `pin_image_url`; emoji fallback). On-device composite hit off-screen-ViewShot-blank + storage-RLS-path + OTA fragility → **pivoted to SERVER-SIDE generation** (service-role upload bypasses RLS), proven, no device/OTA dependency. Durable auto-gen = next build. | Client-composite stacked 3 failure points on one fragile step; server-side is robust + verifiable without a device. | Kevin (device-confirmed), Claude |
