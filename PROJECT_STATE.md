...
> **✅ SHIPPED TO PROD (08/19–20/2026) — `v1.0.0-prod.30`.** The full What's Happening + reproducible-curation bundle promoted through the gated pipeline (Kevin approved the Production gate). Prod carries the **GENERATOR, not staging's scores**: migrations 160–164 + the `score-notability` edge fn, and an in-deploy bootstrap ran the generator over prod's own catalog → **6,181/6,185 items scored fresh + blended → carousels populated (anchors Fore Street/Eventide/Central Provisions eligible).** All prior "Now (08/18)" work below is now live. **Bootstrap-scope note:** the bootstrap scored the FULL prod catalog (all regions, ~$9/45min), not just Portland — scope to the launch region next promotion (see memory [[prod-promotion-whats-happening]]).
>
