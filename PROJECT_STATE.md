...
**Last updated:** 09/04/2026 (Check-in **social map** shipped to prod [PR #219, gated; migs 186-189 + `generate-post-pins` cron] — check-ins on profile+feed, NOT Explore; Phases 1-3 own-profile/friend-profile/feed, privacy = reuse grid/feed query+gate. New sprint set by Kevin: **validate data → notifications → App Store submit → post-with-friends**, each step gating the next. STEP 1 done — 3-market prod health scorecard all LIVE/FRESH/CORRECT [Warwick 93%-past problem fixed]. STEP 2 done — notification current-state inventory. See §2.)

_Prior (08/16/2026): Ingestion restored, curated, self-monitoring — ~2-month API blackout fixed to prod [#123]; notability-gated Ticketmaster live [#125]; rows-based source-liveness monitor [#126]; `complete_normalization_job` enum-cast fix [#129/mig 157]._
**Owner:** Kevin Walter
...
> **🎯 Now (09/04/2026): Social-map shipped to prod; sprint = validate data → notifications → submit → post-with-friends.** Each step gates the next; nothing built until Kevin decides from recon.
> - **Check-in SOCIAL MAP live on prod** (PR #219, gated; migs 186-189 + `generate-post-pins` cron). Check-ins live on profile + feed, **NOT** the Explore map. Phase 1 own-profile grid↔map (all-time life-log), Phase 2 friend-profile map (inside the `canSeeContent` gate), Phase 3 feed list↔map (you+friends, last 30 days). Privacy = reuse the same grid/feed query+gate → no new data surface. Pin backfill 2/2 on prod. Memory [[social-map-checkins-profile-feed]]. The hard UX lesson (inline-map-in-ScrollView + refresh `setLoading` → scroll snaps to top; fix = loader first-load-only + don't re-scroll on nav-return) is recorded there too.
> - **Sprint plan (Kevin, 09/04) — each step gates the next:**
>   1. **Validate data + ingestion (3 markets)** — ✅ DONE (scorecard below).
>   2. **Notifications recon** — ✅ DONE (inventory below). *Kevin decides what to add from the recon before any build.*
>   3. **Build notifications** — staging-first, Kevin device-verifies (must actually receive them), then into the submit build.
>   4. **App Store submit** — fresh prod build from `main` with everything (map + curation + notifications); walk Kevin through submit.
>   5. **Post-with-friends** — the last big feature, AFTER submit. Invite→join→combined-swipeable-post + gold invited-ring (the viral mechanic). Queued mentally, not spec'd.
> - **STEP 1 — 3-market prod health scorecard (09/04): ALL GREEN.**
>   - *Ingestion:* **Ticketmaster** on-schedule (fetched 09-04, 6h cadence, 81 raw/24h). **Web Collector** live (190 raw/24h — tracks via `venue_crawl_state`, so `event_sources.last_fetch_at` is null by design). **Google Places** on-schedule — **NOT drifted**: partition-level interval is weekly (last 08-29, next 09-05), which overrides the misleading source-level 60min; Potsdam+Warwick partitions last succeeded 0-error (525+700 places), Portland partition intentionally disabled. Curated CSVs seeded (Potsdam 242). Monitors all active (`monitor-pipeline-health`/`-error-rates` */30, `-data-quality` daily); `monitoring_daily` rolling through 09-03. **Nothing dark/drifted.**
>   - *Freshness:* Portland 1194 items / 246 upcoming events / +63 in 7d. Warwick 635 / 114 upcoming / +34 in 7d — **the 93%-past problem is FIXED (now ~50/50).** Potsdam 563 / 67 upcoming / +15 in 7d. Newest items 09-03/09-04 across all three.
>   - *Correctness (top carousels):* Portland leads Portland Head Light / Casco Bay ferries (anchor) / PMA / **Scales** ✓ — all notable, local, no sprawl. Warwick leads Wawayanda / Sterling Forest / **Bellvale Farms Creamery** ✓ / Soons Orchards (apple — fall-apt). Potsdam leads in-town notable (Jernabi, Children's Museum); **Remington** (Frederic Remington Art Museum) present + notable + carousel-eligible, ranked below the in-town spots by distance-aware local-first (correct). **ZERO ski/winter items** in any carousel. Potsdam thinness (28 notable) = known sourcing signal, ship as-is.
>   - *Watch (non-blocking):* `null_coord_events` ~675 and slowly growing (auditor already queued a backfill task); Portland GP partition disabled (Portland data is still the richest of the three).
> - **STEP 2 — notification current-state inventory (09/04):** Push-only; **no in-app notification center**. **Working end-to-end:** friend-request, friend-accepted, post-reaction, post-comment (all via `send-notification` edge fn) + event-reminder (cron `send-event-reminders` */15). Infra is real: `expo-notifications`, `push_tokens` table (mig 084) + registration lifecycle, Expo push send path, per-type prefs + `notifications_sent` dedup ledger, permission soft-ask. **Absent:** "friend posted / checked in at X", "things happening near you", any re-engagement/digest — and there is **NO persisted per-user location** (blocks any "near you" query). The permission prompt over-promises "friends going to events nearby" (unimplemented). *Kevin to decide: friend-activity vs re-engagement vs both.*
> - **Auditor:** promoted the Kevin-approved "item-level weak-category samples in the curation audit snapshot" task to the builder queue (`build_tasks` ready, chief). Three newer auditor proposals await Kevin's promote/pass (Portland Arts & Culture card-ready regression 234→189 [pri 95], null-coord backfill [pri 85], enrichment-queue weighting [pri 70]).
>
> _(Prior "Now" blocks preserved below.)_
>
> **🎯 Now (08/18/2026): Two-surface catalog + What's Happening cleanup — built & verified on STAGING, pending Kevin's phone eyeball + one gated prod promotion.** The +6,700-item flood (2,000 restaurants) forced a split: ONE catalog now serves TWO needs — a curated **browse/carousel** surface (tight, high notability bar, region-relative ranking) vs a broad **searchable posting catalog** (queried at post-time, never browsed), separated by `is_carousel_eligible`.
> - **Region-relative carousel cutoff:** top ~25% per intent per region, floor 5 / ceiling 30 (config-tunable). Portland Bite lands at 23.
> - **Blended notability (model + editorial + Google), hallucination-guarded.** Agreement-weighted blend: model-knowledge is the primary discriminator (Google-blind, cached in `model_notability`), editorial (`editorial_mentions` — 98 mentions/3 sources → 27 corroborated) is a booster, Google is existence. **The guard is non-negotiable and holds: 0 model-alone items earn a carousel spot across all 4 intents;** model-notable-but-uncorroborated → flagged, not shipped.
> - **Carousels validated clean across all 4 intents** (Bite/Drink/Outside/See). RC1 (event-nature venues leaking into static intents) routed out — dated→What's Happening, undated→held; State Theatre kept via landmark override. RC2 café split (coffee-first→Drink, brunch→Bite) + a general **model-knowledge type-correction layer** (`type_corrections`, applied through the base mapping).
> - **What's Happening — all issues root-caused + fixed on staging:** (1) ended events excluded (`ends_at` filter in exploreQuery, #145); (2) HTML entities decoded (ingestion `decodeHtmlEntities`, #146 merged + one-time backfill, 0 left); (3) **recurring markets/flea now render as real dated events** — they were `kind='activity'` (so no purple "when" label AND dropped by the Events-only toggle, both of which gate on `kind='event'`). `resolve_market_next_occurrence()` (**migration 160**) graduates them to `kind='event'` with a TZ-correct next occurrence (start+close parsed from `schedule_text`, soonest across all open days) + `recurrence='weekly'`; a nightly cron (03:55, between advance-recurring 03:50 and demote 04:00) rolls them forward and auto-graduates newly-ingested markets. **Persistence proven** (back-date→roll-forward self-heal; `demote_stale_items` can no longer bury them because they're recurring). Regression test added (`formatTileWhen`).
> - **What's Happening is time-windowed** (staging — `src/lib/whatsHappeningWindows.ts` + `groupingEngine`): the flat event list splits into **Happening Now → This Weekend → This Week → Later** — soonest-first within each, recurring series collapsed to one entry, empty windows hidden, day-aware weekend (meaningful Mon–Fri, collapses on Sunday). "Happening Now" is duration-bounded (≤24h, tunable) so always-on programs (a 106-day tour, a month-long exhibit, a season of daysails) don't dominate it. Unit-tested + verified against live data. **This is app-code → needs a staging OTA (and folds into the prod bundle only if reviewed in time).**
> - **⚠️ Reproducibility gap found (blocks the prod gate):** the notability **schema + blend logic** are reproducible (migrations + deterministic SQL), but the **model-knowledge scores + editorial mentions are one-time artifacts** — model verdicts were inline Opus judgment (INSERTed, no generator), editorial was hand-transcribed guide lists. New prod items would sit **unscored**; a new city = redo by hand. Per Kevin's criterion this is the thing to fix **before** prod: build a `score-notability` edge function (Claude API, Google-blind) on the existing enrichment infra + cron so items auto-score, and an editorial-extraction pass. Gate waits on this.
> - **Next:** Kevin eyeballs What's Happening on staging (**market fix is data-only, no deploy; windowing needs a staging OTA**), then the **single gated prod promotion** once reproducibility is fixed. Reproducibility answer: schema + blend-logic + type-correction + market-resolution ship as **migrations** (deterministic, re-runnable); the approved Portland signal (model verdicts + editorial mentions) ships as a **seed migration** (reproducible snapshot, not a one-off staging data load); the model-knowledge regeneration script for *new* cities is the one remaining build. Nothing to prod until Kevin has eyeballed + understands the bundle.
>
> **🎯 Now (08/16/2026): Ingestion pipeline restored, curated, and self-monitoring.** After the nightly builder's first real run, this session ran the ingestion-health arc end-to-end (all through the gated pipeline, Kevin approved every prod gate):
> - **~2-month API blackout root-caused & fixed to prod (#123).** Google Places / Ticketmaster / PredictHQ had produced ~0 rows since mid-June — **two independent mid-June breaks, one masking the other**: (1) an integer **overflow** in `next_fetch_partition`'s exponential backoff so the coordinator picked no partitions (migration **156**); (2) `fetch-coordinator` calling `functions.invoke()` **without forwarding the service-role token** → every ingest 401'd (the guard was added while the coordinator was already frozen, so it was never exercised). Web-collector was never dark (healthy market-shift, not a regression).
> - **Notability-gated Ticketmaster LIVE (#125).** Portland-scoped, free tier. The gate drops tribute/filler (ECLIPSE, Grateful Drag) and keeps events with a named attraction, sports, or an anchor venue — it judges on **act notability + venue, not a venue whitelist**. **First live crawl: 108 fetched → 106 surfaced → 2 dropped, 0 errors; all 106 live in `explore_items`** (coords/category/region/enrichment complete). PredictHQ left **disabled** (no spend, trial key retained); Warwick/Potsdam TM partitions left off.
> - **Source-liveness monitor LIVE (#126).** `monitor-pipeline-health` now alerts (critical) if any ENABLED, actively-configured source produces **zero rows** in its per-type window (3–7d) — it measures OUTPUT, so it also catches a source that fetches but writes nothing. The durable fix for silent source-death; verified it would have caught TM's 60-day silence, and reports 0 dark on a healthy prod.
> - **`complete_normalization_job` enum-cast regression fixed (#129 / migration 157).** Found while verifying the TM crawl: the completion RPC assigned text `CASE` expressions to two enum columns (`job_status`, `ingest_status`) with no cast → `42804` on every call; the worker **swallowed the error**, so jobs sat `running` and were finalized ~29 min late by the `reset_stale_normalization_jobs` sweep. Broken for **all** sources since the enum landed; invisible for ~2 months because ingestion was dark. Casts added; the worker now surfaces any future RPC failure to Sentry. **Proven on prod: completion delta ~1770s → 0s.** Same class as migration 155 (`claim_build_task`).
> - **CI test-isolation hardened (#127).** The `normalize-raw-events` integration test went red on every PR once ingestion resumed (a staging normalization backlog starved its seeded row); the test now back-dates its own job so it's immune to queue depth. Must be test-side — the IT hits live staging.
>
> _(Prior "Now" — map redesign to prod — preserved below.)_
>
> **🗺️ Prior Now (08/07/2026): Map redesign SHIPPED TO PROD; the auth bug is fixed for good.** The whole map was rebuilt on **Mapbox** ([design](docs/design/map_engine_redesign.md) + [map_emoji_pins.md](docs/design/map_emoji_pins.md)): plot **places** (not one pin per event — `src/lib/mapPlaces.ts`), render with a native symbol layer, **emoji pins as bundled Twemoji-on-disc images** (`assets/mappins/` + `src/utils/mapPinImages.ts`), **native collision de-clutter** (`iconAllowOverlap:false` + `symbolSortKey` = notability tiers), snappy select, selected-place persistence on zoom-out, and a **pin-tap accuracy fix** (nearest-feature selection — `src/lib/mapTap.ts`). Validated on staging builds #26/#27, then promoted to prod (see the PROD PROMOTION block below). Also fixed en route: the **geocode regression** (Places API New) and the events-empty issue.
>
> **⚠️ The recurring "Invalid API key" — real root cause found & fixed (08/06).** It was NOT the OTA contamination we first chased. The `staging` build profile (`distribution: store`, no explicit `environment`) **defaulted to the EAS `production` environment**, which stores `EXPO_PUBLIC_SUPABASE_ANON_KEY` as a *secret* (the prod key) — and EAS secrets can't be overridden by eas.json — so every staging build embedded **staging URL + PROD key → 401**. Fixed by pinning `environment` per build profile in eas.json (staging→`preview`, production→`production`). Confirmed by grepping build #25's `.ipa` (prod key present) and build #26's (staging key present, prod absent). See memory [[staging-build-prod-key-env]].
>
> **Release-safety changes made this batch:**
> - **`runtimeVersion` → `{policy: "fingerprint"}`** — native changes bump the runtime version, so an OTA can only reach compatible builds (kills the cross-build OTA-contamination class). [[ota-env-contamination]] amended.
> - **Verified-OTA discipline:** every local staging OTA isolates `.env` + **greps the exported bundle** (staging key present / prod key absent) before publishing. Every build is **grep-verified via its `.ipa`** before we call it fixed. Reusable scripts in scratch (`ship_ota_clean.mjs`, `fetch_ota_bundle.mjs`).
> - **Map changes are testable without a device:** headless Mapbox-GL render (Puppeteer) + a jest test asserting every taxonomy emoji has a bundled pin. Evidence artifact produced for the last review.
>
...
> - **✅ v1.1.0 SUBMITTED to App Store Connect / TestFlight (08/10).** Build #32 (v1.0.0) was **rejected by Apple** (ITMS-90186/90062: the 1.0.0 version train is already approved + closed for new builds). Bumped `app.json` version **1.0.0 → 1.1.0** (PRs #100/#101 through the gated pipeline) → prod **build #33 at v1.1.0** → `submit-production` #2 → **Apple ACCEPTED (submission FINISHED).** Build #33 verified prod-correct (prod key/URL, `com.euda.app`, `EnvBanner` null for prod = no staging banner, version 1.1.0). It lands in TestFlight after Apple processing.
>   - **Still manual (App Store Connect, Kevin):** a *public* App Store release needs a 1.1.0 version listing created in ASC, build #33 attached, "What's New" filled, and **Submit for Review**. `eas submit` only uploads to ASC/TestFlight; it does not create/submit the App Store version.
>
...
|------|----------|-----------|------------|
| [09/04/2026] | **Social-map (check-ins on profile+feed) promoted to prod** (PR #219, gated; migs 186-189 + `generate-post-pins` cron). Phases 1-3 (own-profile / friend-profile / feed maps); check-ins OFF the Explore map. Privacy = reuse the grid/feed query+gate, no new RPC/data surface. Plus **sprint plan set** (data-validate → notifications → App Store submit → post-with-friends, each gating the next); STEP 1 (3-market health) + STEP 2 (notification recon) delivered. | Ships the life-log/social map real users get; the sprint validates all 3 markets on prod BEFORE submit so users don't land on a stale app, and recons notifications before speccing them. | Kevin (approved prod gate + set the sprint), Claude (built + validated) |
| [09/04/2026] | **Auditor "weak-category samples in the audit snapshot" promoted to the builder** (build_tasks `ready`, chief-authored, tier 1). Three newer auditor proposals (Portland A&C card-ready regression, null-coord backfill, enrichment-queue weighting) left `proposed` for Kevin. | Cheap observability win Kevin approved; it makes the auditor's own snapshots diagnose *which* items regress (e.g. the Portland A&C 234→189 finding), not just aggregate %s. | Kevin (approved), Claude (scoped + queued) |
| [08/18/2026] | **Two-surface catalog split + blended notability, shipped to staging.** One catalog → curated browse/carousels (`is_carousel_eligible`, region-relative top-25% cutoff, floor 5 / ceiling 30) vs a broad searchable posting catalog queried at post-time. Carousel eligibility = agreement-weighted blend of model-knowledge (primary, Google-blind) + editorial (booster) + Google (existence), with a hard hallucination guard (**0 model-alone items eligible across all 4 intents**). | The +6,700 / 2,000-restaurant flood proved one catalog can't serve browse + post at once; the blend keeps carousels genuinely curated without fabricating notability. | Kevin (architecture + every calibration call), Claude (built + verified on staging) |
| [08/18/2026] | **Recurring markets/flea graduate to dated events** (migration 160 — `resolve_market_next_occurrence()` + nightly cron). Flip `kind='activity'`→`'event'` with a TZ-correct next occurrence (start+close from `schedule_text`) + `recurrence='weekly'`; only 1-3-day "market days" (daily stands stay stores/activities). | They showed no purple "when" label and vanished under the Events toggle — the app gates both on `kind='event'`. Persistence proven (roll-forward self-heal + demote-safe); reproducible as a migration, not a data artifact. | Kevin (flagged the bug + demanded permanence), Claude (root-caused + fixed + verified) |
| [08/16/2026] | **~2-month API ingestion blackout fixed to prod** (#123, migration 156 + `fetch-coordinator`). Root cause = two mid-June breaks: `next_fetch_partition` backoff integer-overflow (coordinator picked no partitions) + coordinator `functions.invoke()` not forwarding the service-role token (every ingest 401'd). | Google Places / Ticketmaster / PredictHQ had been dark since mid-June; web-collector was healthy throughout (a market-shift, not a regression). | Claude (root-cause + fix), Kevin (approved prod gate) |
| [08/16/2026] | **Notability-gated Ticketmaster live + first Portland crawl** (#125). Free-tier TM, Portland-scoped (`portland-25mi` partition re-enabled, crawls */30). Gate in `ingest-ticketmaster` drops tribute/filler (regex) and keeps events with a named attraction, sports, or an anchor venue — **act notability + venue, not a whitelist**. First live crawl 108→106 surfaced / 2 tributes dropped / 0 errors; 106/106 live in `explore_items`. | The headline-event source, restored after the blackout with curation discipline so tribute/cover acts don't flood the catalog. Verified two ways (prod rows + independent gate replay agree). | Kevin (approved gate approach + the tribute-act call), Claude (built + ran) |
| [08/16/2026] | **PredictHQ stays disabled — no spend.** Integration left in place on the trial key (recorded as `PREDICTHQ_API_KEY` secret), partitions disabled on prod; no further build on it. | Overlaps Ticketmaster coverage; not worth paying for. | Kevin |
| [08/16/2026] | **Source-liveness monitor** (#126) — `monitor-pipeline-health` alerts critical if an ENABLED, actively-configured source produces **0 rows** in its per-type window (3–7d), measuring rows produced (not fetch attempts). Only sources with an enabled partition/target are checked, so an intentionally-paused source is never false-flagged. | The durable fix so a silent source-death can't hide for months again (as TM/PredictHQ did). Kevin flagged it as the most important piece. | Claude (built), Kevin (set as the priority) |
| [08/16/2026] | **`complete_normalization_job` enum-cast fix** (#129, migration 157) + the worker now checks the completion RPC's error. Text `CASE` assigned to enum columns (`job_status`/`ingest_status`) errored `42804` on every call; jobs limped to `done` ~29 min late via the `reset_stale_normalization_jobs` sweep. Same class as migration 155. | Broken for all sources since the enum landed; hidden ~2 months while ingestion was dark, surfaced by re-enabling TM. **Proven on prod: completion delta ~1770s → 0s.** Never swallow a `.rpc()` `.error` in a worker. | Claude (root-cause + fix), Kevin (approved prod gate) |
| [08/16/2026] | **`normalize-raw-events` integration test hardened** (#127) — back-dates its seeded normalization job to the front of the queue so it's claimed first regardless of a staging backlog; asserts on end-state with a short poll. | Restoring ingestion made staging non-inert; the test's "staging is inert" isolation assumption broke, turning the CI gate red on every PR. The fix must be test-side because the IT runs against live staging (a worker change wouldn't take effect until merge). | Claude |
| [08/10/2026] | **Chief-engineer Phase A read-workers built** (branch `feat/phase-a-read-workers`). Three weekly, report-only GitHub Actions workers on the official `anthropics/claude-code-action` + `CLAUDE_CODE_OAUTH_TOKEN` (Kevin's Max subscription — **never** `ANTHROPIC_API_KEY`, which would shadow it into pay-per-token): **Auditor** (Mon; North Star Level-1/2 curation scorecard via new `audit-curation-quality` edge fn + `curation_audit` trend table, migration 153), **Maintainer** (Wed; dep/deprecation/security watch), **Researcher** (Fri; ideas-only, no repo/queue writes). Report to **#euda-chief** (`SLACK_CHIEF_WEBHOOK_URL`). Guardrails: max-turns 30/15/15, timeouts, guaranteed Slack start/finish heartbeat, stop-at-quota-wall, `contents:read` (no commits/PRs). Inert until the two secrets exist; scheduled crons activate once merged to main. | The Quality Audit Loop (North Star §7, "keystone, built first") + the read-half of the scheduled autonomous system, running on subscription with no pay-per-token. Confirmed terms-clean (official action + OAuth token only). The code-writing **builder is deferred** pending a single-task quota measurement (what % of the weekly Max pool one build task costs). | Kevin (approved design + 4 settings), Claude (built) |
| [08/07/2026] | **PROD PROMOTION — `v1.0.0-prod.18`.** Promoted the full ~1-month batch staging→main through the gated pipeline: migrations 150–152 to prod DB, all edge functions (incl. geocode fix), prod build #32 (build-only). Prod build `.ipa` grep-verified (prod key present / staging absent / prod URL / emoji map embedded). | A month of staged, validated work (region + recurrence + Mapbox emoji map + login fix + geocode fix + env-isolation + fingerprint) reached prod together, once the map was confirmed on staging build #27. | Kevin (approved Production gate), Claude (drove) |
| [08/07/2026] | **Pin-tap accuracy fix** — select the feature closest to the tap (not `features[0]`) + guard against the map's deselect racing a pin tap + slightly wider hitbox. Pure logic in `src/lib/mapTap.ts`, unit-tested. | Dense pins under the 44px hitbox returned several features; picking an arbitrary one selected a neighbor ("clicks into another pin"); near-misses hit the map's deselect ("took 3 tries"). | Kevin (reported), Claude |
| [08/07/2026] | **Emoji map pins as bundled images + native collision de-clutter** (branch `feat/map-emoji-icons`, OTA'd to build #26). Each category emoji is a Twemoji glyph on a white/purple disc (`assets/mappins/` + `src/utils/mapPinImages.ts`), drawn by one symbol layer; `iconAllowOverlap:false` gives the zoom de-clutter (notability tiers); selected place stays pinned on zoom-out. Tested before shipping (headless Mapbox-GL render + jest pin-coverage). | Mapbox can't render color emoji as text, and the RN view-snapshot image path blanks on New Arch; static bundled images are the reliable path. Vector dots can't collide, so they couldn't de-clutter. | Kevin (approved look), Claude |
| [08/06/2026] | **"Invalid API key" real root cause + fix.** Staging `store` build defaulted to the EAS `production` environment → inherited the prod `EXPO_PUBLIC_SUPABASE_ANON_KEY` *secret* (uneditable by eas.json) → staging URL + prod key = 401. Fixed by pinning `environment` per build profile in eas.json (staging→`preview`, production→`production`). | The recurring auth bug survived the OTA rollback AND the fingerprint rebuild because the embedded bundle was wrong all along; the OTA contamination was a coincidental red herring. Confirmed by grepping the `.ipa` bundles. | Claude (root-cause + `.ipa` proof), Kevin (validated login) |
| [08/06/2026] | **`runtimeVersion` → `{policy: "fingerprint"}`** + verified-OTA/`.ipa`-grep discipline. | appVersion policy pinned every 1.0.0 build to one OTA lane, letting a stale OTA reach an unrelated native build; fingerprint isolates by native compatibility. Never call a map/build fix done without grepping the served bundle / `.ipa` for the right key. | Kevin (approved), Claude |
| [08/05/2026] | **Map engine → Mapbox** (`@rnmapbox/maps`), replacing react-native-maps. Plot **places** (one pin per venue/coord — `src/lib/mapPlaces.ts`), native symbol layer. Tokens (public + download) live as **EAS project secrets**, never in eas.json (push protection rejects `pk.*`) — memory [[mapbox-tokens-eas-secrets]]. | react-native-maps custom markers are fundamentally fragile on New Arch (pop-in/out, tap-blank); plotting one pin per event collapsed 139 events at 25 coords into a few visible pins. | Kevin (chose Mapbox + funded tokens), Claude |
| [08/04/2026] | **Geocode regression fixed** — `normalize-raw-events` rewired from the (disabled) Geocoding API to **Places API (New) `places:searchText`**. Prod was geocoding 0% of last-7d events. | Geocoding API + legacy Places API return REQUEST_DENIED; Places API (New) is the enabled one. Durable fix for prod's empty events map. | Claude |
...
### Deployed
- **PROD (latest, 08/16/2026) — ingestion restoration + curation, backend-only.** Prod DB now at **migration 157** (156 `next_fetch_partition` overflow fix, 157 `complete_normalization_job` enum-cast fix). Redeployed edge functions: `fetch-coordinator` (service-role auth header), `ingest-ticketmaster` (notability gate), `monitor-pipeline-health` (rows-based source-liveness), `normalize-raw-events` (checks the complete-RPC error). Ticketmaster `portland-25mi` partition **re-enabled** (crawls */30 via cron jobid 7); first gated crawl live (106 events in `explore_items`, completing instantly). App build unchanged (still the `v1.1.0` / prod.18-era binary — these are all backend changes). Shipped via `deploy-production` runs behind Kevin's `Production` gate.
- **PROD (08/07/2026): `v1.0.0-prod.18`** — prod DB was at **migration 152**; all edge functions redeployed (incl. geocode fix on Places API New); prod app **build #32** built (build-only, **not yet App-Store-submitted**). Carries: region model + recurrence collapse + Mapbox emoji map (+ tap fix) + login crash fix + geocode fix + eas.json env-isolation + fingerprint runtimeVersion. Build #32 `.ipa` verified: prod key + prod URL + emoji pins embedded. **Follow-up pending:** prod geocode backfill (153 upcoming events missing coords) + App Store submit — both Kevin's call.
- **STAGING (current): build #27** (native, embeds emoji map) + latest via OTA rtv `7bef7ff`. Staging DB at migration 152.
...
### In flight
- **App Store: v1.1.0 build #33 submitted + Apple-accepted → TestFlight** (08/10). Public App Store release still needs the manual App Store Connect step (create 1.1.0 listing, attach #33, What's New, Submit for Review). Prod DB/functions unchanged since prod.18; #33 is the same code as #32 with only the version bump 1.0.0→1.1.0.
...
- **Ticketmaster (08/16):** `portland-25mi` fetch_partition **ENABLED** (Portland-scoped, notability-gated pilot); `warwick-40mi` + `potsdam-50mi` left **disabled**.
- **PredictHQ (08/16):** **disabled** on prod (all partitions off). Trial key retained as `PREDICTHQ_API_KEY`; no spend, no further build per Kevin's call.
- **Source-liveness monitor** watches every ENABLED source (see §Recent Decisions 08/16) — re-enabling a partition automatically brings that source under the 0-rows alarm.
...
- **⚠️ EAS environment pinning (eas.json, fixed 08/06):** each build profile now sets `environment` explicitly — `staging`→`preview`, `production`→`production`. Without it a `store`-distribution build defaults to the EAS `production` environment and inherits the prod `EXPO_PUBLIC_SUPABASE_ANON_KEY` *secret* (which eas.json can't override) → staging build ships the PROD key → "Invalid API key". **Do not add a supabase secret to the `preview` EAS environment**, and never remove these `environment` fields. Verify any build by grepping its `.ipa` bundle: staging key `oE88rxo9…` present / prod key `sb_publishable_mxVui…` absent. See [[staging-build-prod-key-env]].

### ⚠️ Track A environment rule — data plane = PROD, build plane = STAGING (08/20/2026)
**The durable rule that ends the recurring staging/prod conflation** (it has cost cycles repeatedly — e.g. the propose loop was built on staging while the builder reads prod). The autonomous loop has two planes:
- **DATA PLANE = PROD.** Track A RUNTIME STATE lives on prod: the `build_tasks` queue, the auditor's audit target (the real catalog it scores), the propose infra (`proposed` rows), Kevin's promote gate, and the builder's claim-source. `run_night.mjs` claims from prod **by design** — the prod service-role key lives ONLY in the orchestrator. Prod is the single source of truth for autonomous work.
- **BUILD PLANE = STAGING.** Track A CODE/SCHEMA (migrations, edge functions, workflow YAML) develops on staging → promotes to prod via the gated pipeline, like all code. Each builder task's `claude -p` runs with a **scrubbed env (no prod keys) + staging creds**, so building/testing happens against staging safely, and its output is needs_kevin PRs (staging→prod).
- **The rule:** RUNTIME STATE (queue rows, proposals, audit, promotions) is **always PROD**; CODE/SCHEMA (functions, migrations, workflows) is **staging→prod**. Never operate the live queue/proposals on staging.

### ⚠️ Migrations flow feature → staging → main, ALWAYS (08/25/2026)
**The hard rule that ends the staging-drift class of bugs** (it cost a full recovery once: Phase-3 PRs failed CI because migrations were merged straight to `main` and staging was hand-patched, so the integration tests — which run against **live staging** — had no clean DB to test against).
- **Migration-bearing work goes to the `staging` branch FIRST.** `deploy-staging.yml` applies it to the staging DB via the pipeline (`supabase db push`), the integration tests validate against a real staging, THEN it promotes to `main` (→ gated prod). **Never merge a migration straight to `main`.**
- **NEVER hand-apply migrations to staging** (no raw-SQL `q.mjs`/mgmt-API applies to "validate"). That is exactly what drifts staging's recorded history from its actual objects and breaks the integration gate. Let `db push` (the pipeline) be the only thing that writes migrations to staging. Validate by pushing to `staging` and reading the deploy-staging result, not by hand-applying.
- **Guardrail (beats the written rule):** `guard-migrations-via-staging.yml` fails any PR to `main` that adds a `supabase/migrations/*.sql` file not already present on the `staging` branch. So a migration physically cannot reach `main` without going through staging first. Make it a required check in branch protection.
- Integration tests run against **live staging** (`test.yml` integration job — no `db push` of the PR's own migrations), so staging MUST already have the migration for a behavior-changing migration's test to pass. This is *why* the order is non-negotiable.

### ⚠️ Phase 3 feature/native work is STAGING-FIRST → Kevin device-tests → THEN prod (08/26/2026)
**Kevin's standing rule for all Phase 3 feature / UI / native work.** (The post-first *foundation* was shipped straight to prod in one motion — additive + T2-verified that the existing posting path is untouched, so acceptable that once — but that is the OPPOSITE of the rhythm Kevin wants for work he needs to feel on-device.)
- **The rhythm: feature → `staging` build/OTA → Kevin tests it on his device → Kevin approves → promote to prod.** Never build-and-ship-to-prod in one motion for feature work. The **"Kevin tests on staging" step is non-skippable** for native/UI work.
- **Every prod deploy waits for Kevin's EXPLICIT manual approval** at the `Production` environment gate — every time, no exceptions. The chief does **not** approve prod deploys on Kevin's behalf unless Kevin explicitly says so for that specific deploy. (The chief's `GITHUB_TOKEN` **is** `kevinwalter22`, so it *can* satisfy the gate — it must not, except on explicit instruction. The gate itself is verified sound: `Production` env has `required_reviewers = kevinwalter22`; it cannot auto-proceed.)
- **Especially the native crown jewels — T5 (camera→place-picker→details), T7 (location puck), T8 (map parity): MUST be device-tested on staging before they touch prod users.** A broken camera flow on staging is fine; on prod it is not acceptable.
- Mechanics: land on `staging` → `deploy-staging` OTAs/builds it → Kevin reviews on his staging build → on his OK, promote `staging → main` (→ gated prod; Kevin approves the gate himself). Hard-rule version of the [[builder-review-flow]] default.

### Task routing — builder vs chief (08/20/2026)
The bounded Sonnet overnight builder reliably finishes **well-scoped, browser-testable, VISUAL/bounded** tasks (the carousel work proved it). It does **not** finish **open-ended investigation** — "investigate X" tasks have no clear "done" and blow the turn cap (the sub_category + midnight cleanup tasks both hit it; done chief-side in #158). **Rule: only bounded/testable/visual work is builder-queued; investigative / data-quality / pipeline tasks are chief-run.** Keep the queue honest about what the builder can actually finish. See [[builder-task-cost]], [[scheduled-autonomous-dev]].
...
- **Cost optimization (06/18/2026):** the critique pass re-sends the full HTML (~doubles input tokens), so it is now **confidence-gated** — skipped when every surviving candidate is already dated + verbatim-verified (the critique almost never drops those); undated/partially-rejected batches still get critiqued. `opts.forceCritique` overrides; `diagnostics.critique_skipped` reports it. Measured on the 10-fixture suite: input tokens −49% (210K→108K), cost/crawl −21% ($0.029→$0.023), **recall/precision unchanged at 88.9%/97.0%, zero hallucinations**. This stacks on the pre-existing upstream gating (collector_page_cache content-hash + ETag/304: the LLM only runs on *changed* pages whose deterministic extraction yielded < 2 candidates — unchanged pages already cost $0). **Prompt caching — evaluated and DROPPED (06/28/2026), don't relitigate.** Measured via `count_tokens` on Haiku 4.5 (cache floor **4,096 tokens**): extraction system prompt = **518 tok**, critique system prompt = **219 tok** (both ~8–19× below the floor → no cross-call system caching). The only structural reuse is the extraction→critique pair re-sending the same HTML, but (i) preprocessed HTML is often itself below 4,096 (e.g. VisitPortland = **1,774 tok**) so the cache can't even engage, and (ii) where HTML is large, the confidence-gate already cut critique input ~49% → critique fires on only ~single-digit % of crawls, far below the **27.8% break-even** (5-min TTL: 1.25× write + 0.1× read vs 2×); below break-even the write premium on the common critique-skipped path makes caching a net loss. Same conclusion as enrichment caching (enrichment prompt 3,416 tok < 4,096). If asked again: the answer is no — the numbers are here.
